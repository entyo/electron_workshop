...
### 3. WebページをそのままElectron Appにしてみる
本当はちょっとしたブラウザを作ってみよう！とかにしたかったんですが、[大家さんは思春期! (1) (まんがタイムコミックス)](http://www.amazon.co.jp/dp/4832252356)を読む暇もないくらいには忙しかったんですごめんなさい。

最後に、otintin.comをElectron Appにしてもらおうと思います。端末で以下のコマンドを入力してください。
````
npm install -g nativefier
nativefier otintin.com
````
すると、カレントディレクトリに"otintin-ほげほげ"(ほげほげはOSに依存)というディレクトリができているはずです。そこへ移動して、実行可能ファイルを実行してみてください。

![otintin.com](assets/img/otintin.png)

このように、otintin.comのトップページがデスクトップアプリケーションになりました。Atomもそうでしたが、Electronはこのようにビルドターゲットを指定して実行可能ファイルを生成することも当然できます。

また、あくまでページをElectronでラップしただけなので、トップページから他のページに飛んでも戻ることができません。だから簡易ブラウザを作りたかったんだよなぁ…
